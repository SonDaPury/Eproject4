  ...
  const handlePaid = async (price) => {
    ...
    return (
      ...
      <LoadingBackdrop open={cartLoading || orderLoading} />
        ...
        <input type="text" value={data.length > 0 ? data[0].username : ""} placeholder="Tên người dùng" disabled />
        <input type="email" value={data.length > 0 ? data[0].email : ""} placeholder="Email Address" disabled />
        </div>
        <div className="right-column">
        <h2>Courses ({data.length > 0 ? data[0].sources.length : 0})</h2>
        {data.length > 0 && data[0].sources.map((item, index) => (
          <div className="flex2" key={index}>
            <div className="course" >
              <img src={item.thumbnail ? item.thumbnail : "https://www.invert.vn/media/uploads/uploads/2022/12/03193534-2-anh-gai-xinh-diu-dang.jpeg"} alt="Course 2" />
            <div>
                <h3>{item.sourceName}</h3>
                <p>{item.price} VND</p>
          </div>
            </div>
            <button className="delete-button" onClick={() => handleDelete(item.sourceID)}>Xóa</button>
          </div>
        ))}
        ...
        {data.length > 0 && (
          <button className="complete-payment" onClick={() => handlePaid(data[0].totalPrice)}>
            Thanh toán
          </button>
        )}      </div>
      </div>
