...
export const cartServices = () => {
    ...
    const GetCardByUserID = async (data) => {
        try{
            const res = await callApi(`/Cart/GetCart?userID=${data}`, "get", data, null, false);
            return res;
        }catch(err){
            console.log(err);
        }
    ...
    const DeleteCart = async (sourceID, userID) => {
        try{
            const res = await callApi(`/Cart/DeletCart?SourceID=${sourceID}&userid=${userID}`, "delete", null, null, false);
            return res;
        }catch(err){
            console.log(err);
        }
