  ...
  <Box
    ...
    <Box sx={{ maxWidth: "1320px", margin: "auto" }}>
      ...
      <Grid container spacing={2} textAlign="center" sx={{ px: 2, py: 3 }}>
        {courses.map((course) => (
          <Grid item xs={12} sm={6} md={4} lg={2.4} key={course.source.id}>
            <CardCourse
              id={course.source.id}
              path={`/course/${course.source.id}`}
              category={course.source.topic}
              price={`$${course.source.price}`}
              title={course.source.title}
              rating={course.source.rating}
              students={course.source.views}
              image={course.source.thumbnail == "" ? "https://via.placeholder.com/300" : course.source.thumbnail}
            />
          </Grid>
        ))}
      </Grid>
